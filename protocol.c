...
enum { BIG_MAX_BODY = 65579 }; // [cite: 458]
...
typedef struct {
    ...
    uint8_t channel_id;
} BodyMessageCreateReq; // 43 bytes [cite: 301]
...
typedef struct {
    ...
    uint8_t channel_id;
    uint8_t sender_id;
} BodyMessageReadRes; // 44 bytes [cite: 317]

#pragma pack(pop)

/* Portable 64-bit Endian Swapping for Timestamp [cite: 171] */
...
#define MAX_CHANNEL_MEMBERS 255 // Allows full capacity defined by 1-byte id length [cite: 279]
...
static void channels_init_once(void) {
    ...
    // Milestone 2: Hardcode initial channel "milestone2"
...
int protocol_handle_one(int client_fd, user_db_t *db) {
    ...
    if (h.type == MSG_USER_CREATE_REQ) {
        ...
        if (st == UDB_OK) {
            ...
            /* Milestone 2: Automatically add to milestone2 channel on account creation */
            pthread_mutex_lock(&g_channels_mu);
    ...
    if (h.type == MSG_USER_UPDATE_REQ) {
        ...
        if (req.user_status == 1) { // UserStatus = Online [cite: 252, 253]
            client_set_fd(uid, client_fd);
            /* Milestone 2: Ensure user is dynamically added to milestone2 channel on login */
            pthread_mutex_lock(&g_channels_mu);
        ...
        memset(req.password16, 0, 16); // Secure response by zeroing out Auth credentials [cite: 192]
    ...
    if (h.type == MSG_MESSAGE_CREATE_REQ) {
        ...
        if (!channel_has_member_locked(&g_channels[ch_idx], sender_uid)) {
            pthread_mutex_unlock(&g_channels_mu); free(body); 
        ...
        for(int i=0; i<MAX_HISTORY; i++) {
            if (g_msg_history[i].channel_id == req.channel_id && g_msg_history[i].sender_id == sender_uid && g_msg_history[i].timestamp == ts) {
                ...
                // Reject the message if the combination of ChannelId, UserId, and Timestamp already exists 
        ...
        // Send confirmation back to Sender with wiped out Auth credentials [cite: 192, 301]
        uint8_t *resp_body = malloc(body_len); memcpy(resp_body, body, body_len); memset(resp_body, 0, 32); 
        int ret = send_response_raw(client_fd, MSG_MESSAGE_CREATE_RES, ST_OK, resp_body, body_len);
        free(resp_body);

        /* Broadcast Message Read Response (0x33) to target channel [cite: 307] */
        uint32_t read_len = 44 + msg_len; // 44 bytes prefix for Message Read response body [cite: 317]
        uint8_t *read_body = calloc(1, read_len);
        
        memcpy(read_body + 32, body + 32, 8); // Copy Timestamp
        memcpy(read_body + 40, body + 40, 2); // Copy MessageLen
        read_body[42] = req.channel_id;
        read_body[43] = sender_uid;
        memcpy(read_body + 44, body + 43, msg_len); // Copy payload

        // Unsolicited response pushed out to all active members of the target channel [cite: 324]
