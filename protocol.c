...
typedef struct { uint8_t ipv4[4]; uint8_t id; } Body5;

typedef struct { uint8_t username16[16]; uint8_t password16[16]; uint8_t user_id; } BodyUserCreate;

typedef struct { uint8_t username16[16]; uint8_t password16[16]; uint8_t ipv4[4]; uint8_t user_status; } BodyUserUpdate;

typedef struct { uint8_t auth_username16[16]; uint8_t auth_password16[16]; uint8_t username16[16]; uint8_t target_user_id; } BodyUserRead;

typedef struct { uint8_t auth_username16[16]; uint8_t auth_password16[16]; uint8_t channel_name16[16]; uint8_t channel_id; } BodyChannelCreate;

typedef struct { uint8_t auth_username16[16]; uint8_t auth_password16[16]; uint8_t channel_name16[16]; } BodyChannelReadReq;

typedef struct { uint8_t auth_username16[16]; uint8_t auth_password16[16]; } BodyAuth32;

typedef struct { uint8_t auth_username16[16]; uint8_t auth_password16[16]; uint8_t channel_id_len; } BodyChannelsReadReq33;
...
/* Message History (for Deduplication / ST_AlreadyExists) */
...
static bool channel_create_with_owner(const uint8_t name16[16], uint8_t owner_user_id, uint8_t *out_channel_id) {
    ...
    if (channel_find_index_by_name_locked(name16) >= 0) {
        pthread_mutex_unlock(&g_channels_mu); return false;
    }

    for (int i = 0; i < MAX_CHANNELS; i++) {
        if (!g_channels[i].in_use) { slot = i; break; }
    }

    if (slot < 0) { pthread_mutex_unlock(&g_channels_mu); return false; }
...
static bool is_valid_type_combo(uint8_t type) {
    ...
    switch (r) {
        ...
        case RES_CHANNEL: return (a == CRUD_CREATE) || (a == CRUD_READ);
...
static void username16_to_key(const uint8_t in[16], char out[17]) {
    memcpy(out, in, 16); out[16] = '\0';
}

static bool udb_is_exists(udb_status_t st) { return st == UDB_ERR_EXISTS; }

static bool auth_ok16(user_db_t *db, const uint8_t username16[16], const uint8_t password16[16]) {
    char key[17]; username16_to_key(username16, key);
    void *stored = NULL; size_t stored_len = 0;
    
    pthread_mutex_lock(&g_db_mu);
    udb_status_t st = user_db_get(db, key, &stored, &stored_len);
    pthread_mutex_unlock(&g_db_mu);

    if (st != UDB_OK || !stored) { free(stored); return false; }
...
static int send_response_raw(int fd, uint8_t type, uint8_t status, const void *body, uint32_t body_len) {
    ...
    if (body_len > 0 && body) if (write_exact(fd, body, body_len) < 0) return -1;
    return 0;
}
...
int protocol_handle_one(int client_fd, user_db_t *db) {
    ...
    if (h.version != PROTO_V2) return send_response_keepalive(client_fd, resp_type, ST_InvalidVersion, NULL, 0) - 2;
    ...
    if (body_len > BIG_MAX_BODY) return send_response_keepalive(client_fd, resp_type, ST_MessageTooLarge, NULL, 0) - 2;

    if (body_len > 0) {
        body = (uint8_t *)malloc(body_len);
        if (!body) return send_response_keepalive(client_fd, resp_type, ST_ResourceExhausted, NULL, 0) - 2;
        if (read_exact(client_fd, body, body_len) <= 0) { free(body); return -1; }
    }
    ...
    if (h.type == MSG_SERVER_REG_REQ) {
        if (body_len != sizeof(Body5)) { free(body); return send_response_keepalive(client_fd, MSG_SERVER_REG_RES, ST_InvalidSize, NULL, 0); }
    ...
    if (h.type == MSG_ACTIVATE_REQ || h.type == MSG_OLD_SERVER_ACTIVATE_REQ) {
        ...
        if (body_len != sizeof(Body5)) { free(body); return send_response_keepalive(client_fd, t, ST_InvalidSize, NULL, 0); }
    ...
    if (h.type == MSG_GET_ACTIVE_REQ || h.type == MSG_OLD_SERVER_GET_REQ) {
        ...
        free(body);
        Body5 res; memset(&res, 0, sizeof(res));
        if (!active_get(&res)) return send_response_keepalive(client_fd, t, ST_NotRegistered, NULL, 0);
        return send_response_keepalive(client_fd, t, ST_OK, &res, sizeof(res));
    }

    /* ================= USER CREATE ================= */
    if (h.type == MSG_USER_CREATE_REQ) {
        if (body_len != sizeof(BodyUserCreate)) { free(body); return send_response_keepalive(client_fd, MSG_USER_CREATE_RES, ST_InvalidSize, NULL, 0); }
        BodyUserCreate req; memcpy(&req, body, sizeof(req)); free(body);
        char key[17]; username16_to_key(req.username16, key);
    ...
    if (h.type == MSG_USER_UPDATE_REQ) {
        if (body_len != sizeof(BodyUserUpdate)) { free(body); return send_response_keepalive(client_fd, MSG_USER_UPDATE_RES, ST_InvalidSize, NULL, 0); }
        BodyUserUpdate req; memcpy(&req, body, sizeof(req)); free(body);
        
        if (!auth_ok16(db, req.username16, req.password16)) return send_response_keepalive(client_fd, MSG_USER_UPDATE_RES, ST_InvalidCreds, NULL, 0);
        if (req.user_status > 1) return send_response_keepalive(client_fd, MSG_USER_UPDATE_RES, ST_MalformedRequest, NULL, 0);

        uint8_t uid = 0;
        if (!uidmap_find_by_username(req.username16, &uid)) { // Repair DB->Memory map on restart
    ...
    if (h.type == MSG_USER_READ_REQ) {
        if (body_len != sizeof(BodyUserRead)) { free(body); return send_response_keepalive(client_fd, MSG_USER_READ_RES, ST_InvalidSize, NULL, 0); }
        BodyUserRead req; memcpy(&req, body, sizeof(req)); free(body);
        
        if (!auth_ok16(db, req.auth_username16, req.auth_password16)) return send_response_keepalive(client_fd, MSG_USER_READ_RES, ST_InvalidCreds, NULL, 0);
        
        uint8_t uname16[16];
        if (!uidmap_get(req.target_user_id, uname16)) return send_response_keepalive(client_fd, MSG_USER_READ_RES, ST_NotFound, NULL, 0);
        
        memset(&req, 0, sizeof(req)); // Zero out auth
    ...
    if (h.type == MSG_CHANNEL_CREATE_REQ) {
        if (body_len != sizeof(BodyChannelCreate)) { free(body); return send_response_keepalive(client_fd, MSG_CHANNEL_CREATE_RES, ST_InvalidSize, NULL, 0); }
        BodyChannelCreate req; memcpy(&req, body, sizeof(req)); free(body);

        if (!auth_ok16(db, req.auth_username16, req.auth_password16)) return send_response_keepalive(client_fd, MSG_CHANNEL_CREATE_RES, ST_InvalidCreds, NULL, 0);
        if (all_zero16(req.channel_name16)) return send_response_keepalive(client_fd, MSG_CHANNEL_CREATE_RES, ST_MalformedRequest, NULL, 0);

        uint8_t owner_uid = 0;
        if (!uidmap_find_by_username(req.auth_username16, &owner_uid)) return send_response_keepalive(client_fd, MSG_CHANNEL_CREATE_RES, ST_NotRegistered, NULL, 0);

        if (!channel_create_with_owner(req.channel_name16, owner_uid, &req.channel_id)) {
            return send_response_keepalive(client_fd, MSG_CHANNEL_CREATE_RES, ST_AlreadyExists, NULL, 0);
        }

        memset(req.auth_password16, 0, 16);
        return send_response_keepalive(client_fd, MSG_CHANNEL_CREATE_RES, ST_OK, &req, sizeof(req));
    }

    /* ================= CHANNEL READ / CHANNELS READ ================= */
    if (h.type == MSG_CHANNELS_READ_REQ) {
        if (body_len < sizeof(BodyAuth32)) { free(body); return send_response_keepalive(client_fd, MSG_CHANNELS_READ_RES, ST_InvalidSize, NULL, 0); }
        BodyAuth32 auth; memcpy(&auth, body, sizeof(auth)); free(body);

        if (!auth_ok16(db, auth.auth_username16, auth.auth_password16)) return send_response_keepalive(client_fd, MSG_CHANNELS_READ_RES, ST_InvalidCreds, NULL, 0);

        uint8_t out[32 + 1 + 255]; memset(out, 0, 32); // zero auth prefix
    ...
    if (h.type == MSG_CHANNEL_READ_REQ) {
        if (body_len != sizeof(BodyChannelReadReq)) { free(body); return send_response_keepalive(client_fd, MSG_CHANNEL_READ_RES, ST_InvalidSize, NULL, 0); }
        BodyChannelReadReq req; memcpy(&req, body, sizeof(req)); free(body);

        if (!auth_ok16(db, req.auth_username16, req.auth_password16)) return send_response_keepalive(client_fd, MSG_CHANNEL_READ_RES, ST_InvalidCreds, NULL, 0);

        int idx; pthread_mutex_lock(&g_channels_mu);
        idx = channel_find_index_by_name_locked(req.channel_name16);
        if (idx < 0) { pthread_mutex_unlock(&g_channels_mu); return send_response_keepalive(client_fd, MSG_CHANNEL_READ_RES, ST_NotFound, NULL, 0); }

        uint8_t out[50 + MAX_CHANNEL_MEMBERS]; memset(out, 0, 32);
        memcpy(&out[32], g_channels[idx].channel_name16, 16);
        out[48] = g_channels[idx].channel_id;
        out[49] = g_channels[idx].member_count;
        if (out[49] > 0) memcpy(&out[50], g_channels[idx].member_ids, out[49]);
        pthread_mutex_unlock(&g_channels_mu);

        return send_response_keepalive(client_fd, MSG_CHANNEL_READ_RES, ST_OK, out, 50 + out[49]);
    }

    /* ================= MESSAGE CREATE (0x30) ================= */
    if (h.type == MSG_MESSAGE_CREATE_REQ) {
        if (body_len < sizeof(BodyMessageCreateReq)) { free(body); return send_response_keepalive(client_fd, MSG_MESSAGE_CREATE_RES, ST_InvalidSize, NULL, 0); }
        BodyMessageCreateReq req; memcpy(&req, body, sizeof(req));
        
        uint16_t msg_len = ntohs(req.message_len);
        if (body_len != sizeof(req) + msg_len) { free(body); return send_response_keepalive(client_fd, MSG_MESSAGE_CREATE_RES, ST_InvalidSize, NULL, 0); }
        
        if (!auth_ok16(db, req.auth_username16, req.auth_password16)) { free(body); return send_response_keepalive(client_fd, MSG_MESSAGE_CREATE_RES, ST_InvalidCreds, NULL, 0); }

        uint8_t sender_uid = 0;
        if (!uidmap_find_by_username(req.auth_username16, &sender_uid)) { free(body); return send_response_keepalive(client_fd, MSG_MESSAGE_CREATE_RES, ST_InternalError, NULL, 0); }

        pthread_mutex_lock(&g_channels_mu);
        int ch_idx = -1;
        for (int i=0; i<MAX_CHANNELS; i++) {
            if (g_channels[i].in_use && g_channels[i].channel_id == req.channel_id) { ch_idx = i; break; }
        }
        ...
        for(int i=0; i<MAX_HISTORY; i++) {
            if (g_msg_history[i].channel_id == req.channel_id && g_msg_history[i].sender_id == sender_uid && g_msg_history[i].timestamp == ts) {
                pthread_mutex_unlock(&g_msg_hist_mu); pthread_mutex_unlock(&g_channels_mu); free(body);
        ...
        g_msg_history[h_idx].channel_id = req.channel_id; g_msg_history[h_idx].sender_id = sender_uid; g_msg_history[h_idx].timestamp = ts;
        ...
        uint8_t *resp_body = malloc(body_len); memcpy(resp_body, body, body_len); memset(resp_body, 0, 32); // Echo body, wipe auth
        int ret = send_response_raw(client_fd, MSG_MESSAGE_CREATE_RES, ST_OK, resp_body, body_len);
        free(resp_body);

        /* Broadcast Message Read Response (0x33) to target channel */
        uint32_t read_len = sizeof(BodyMessageReadRes) + msg_len;
        uint8_t *read_body = calloc(1, read_len);
        memcpy(read_body + 32, &req.timestamp, 8);
        memcpy(read_body + 40, &req.message_len, 2);
        read_body[42] = req.channel_id;
        read_body[43] = sender_uid;
        memcpy(read_body + 44, body + sizeof(req), msg_len);

        for (int i = 0; i < g_channels[ch_idx].member_count; i++) {
            int mem_fd = client_get_fd(g_channels[ch_idx].member_ids[i]);
            if (mem_fd >= 0) send_response_raw(mem_fd, MSG_MESSAGE_READ_RES, ST_OK, read_body, read_len);
        }

        free(read_body); pthread_mutex_unlock(&g_channels_mu); free(body);
        return ret < 0 ? -1 : 1;
    }

    /* ================= MESSAGE READ (0x32) ================= */
    if (h.type == MSG_MESSAGE_READ_REQ) {
        if (body_len < sizeof(BodyMessageReadRes)) { free(body); return send_response_keepalive(client_fd, MSG_MESSAGE_READ_RES, ST_InvalidSize, NULL, 0); }
        BodyMessageReadRes req; memcpy(&req, body, sizeof(req)); free(body);

        if (!auth_ok16(db, req.auth_username16, req.auth_password16)) return send_response_keepalive(client_fd, MSG_MESSAGE_READ_RES, ST_InvalidCreds, NULL, 0);

        pthread_mutex_lock(&g_msg_hist_mu);
        bool found = false; HistoricalMsg m;
        for (int i=0; i<MAX_HISTORY; i++) {
            if (g_msg_history[i].channel_id == req.channel_id && g_msg_history[i].timestamp == my_ntohll(req.timestamp)) {
                m = g_msg_history[i]; found = true; break;
            }
        }
        pthread_mutex_unlock(&g_msg_hist_mu);

        if (!found) return send_response_keepalive(client_fd, MSG_MESSAGE_READ_RES, ST_NotFound, NULL, 0);

        uint32_t rlen = sizeof(BodyMessageReadRes) + m.msg_len;
        uint8_t *rbody = calloc(1, rlen);
        uint64_t net_ts = my_htonll(m.timestamp); memcpy(rbody + 32, &net_ts, 8);
        uint16_t net_len = htons(m.msg_len); memcpy(rbody + 40, &net_len, 2);
        rbody[42] = m.channel_id; rbody[43] = m.sender_id;
        memcpy(rbody + 44, m.msg, m.msg_len);

        int ret = send_response_raw(client_fd, MSG_MESSAGE_READ_RES, ST_OK, rbody, rlen);
        free(rbody); return ret < 0 ? -1 : 1;
    }
